...
public class BasicAirportTest {
	...
	public static void main (String [] args) {
		...
		provar("m�tode getCapacity. Aeroport buit");
		try {
		...
		provar("m�tode size. Aeroport buit");
		try {
			if (ai.size()!=0) {
		...
		provar("m�tode isFull. Aeroport buit");
		try {
			if (ai.isFull()) {
		...
		provar("m�tode isEmpty. Aeroport buit");
		try {
			if (!ai.isEmpty()) {
		...
		informar("aterratge aparentment exit�s");
		...
		provar("m�tode size. Aeroport amb avions");
		try {
			if (ai.size()!=5) {
				notificarError("size no ha proporcionat el resultat esperat", SORTIR);
			}
		...
		provar("m�tode isEmpty. Aeroport amb avions");
		try {
			if (ai.isEmpty()) {
				notificarError("isEmpty no ha proporcionat el resultat esperat", SORTIR);
			}
